...
xdot(7) = c3*c2*u + (c3*s2*s1-s3*c1)*v + (s3*s1+c3*c1*s2)*w;
xdot(8) = s3*c2*u + (c1*c3+s1*s2*s3)*v + (c1*s2*s3-c3*s1)*w;
xdot(9) = -s2*u + c2*s1*v + c1*c2*w;

% Calculate inertial frame orientations
xdot(10) = p + (q*s1 + r*c1)*t2;
xdot(11) = q*c1 - r*s1;
xdot(12) = (q*s1 + r*c1)*sec(theta);
